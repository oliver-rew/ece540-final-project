...
*/
    ...
    // since wishbone is a single bus and AHB lite is 2-cycle pipelined, we need
    // to delay HADDR, HWRITE, and HSEL on the first cycle to align with HWDATA 
    // on the second cycle, which is when the actual operation occurs 
    always @ (posedge HCLK) 
    begin
    HADDR_d  <= HADDR;
    HWRITE_d <= HWRITE;
    ...
    i2c_master_top i2c(
        .wb_clk_i(i2c_clk), 
        .wb_rst_i(reset),           //this reset is controlled by a reg
        .arst_i(HRESETn),           //active low
        ...
        //HSEL_d should work for strobe and cycle because the writes should only be one clock cycle
        .wb_stb_i(HSEL_d),      
        .wb_cyc_i(HSEL_d),      
